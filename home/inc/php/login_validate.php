...
if (isset($_POST["loginbtn"])) {
		...
		$selectdata = $dircon->prepare("SELECT * FROM crypto_users WHERE email = ? AND pwd = ? AND stat = ?");
		$bindthem = $selectdata->bind_param("sss", $email, $password, $stat);

		$executelogin = $selectdata->execute();
		// $selectdata->bind_result($email, $password);
